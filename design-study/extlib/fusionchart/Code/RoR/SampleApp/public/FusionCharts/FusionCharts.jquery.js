...
/*members ":", Event, FusionCharts, _fcDrillDownLevel, add,
    addEventListener, appendChild, appendFusionCharts, attrFusionCharts,
    call, clone, cloneFusionCharts, configureLink, containerElementId,
    convertToFusionCharts, core, createElement, createTextNode,
    dataConfiguration, dataFormat, dataSource, delayedRender,
    drillDownFusionChartsTo, each, eventType, expr, extend, filter, find,
    fn, get, getChartAttribute, height, id, insertBefore,
    insertFusionCharts, insertMode, length, options, parentNode,
    prependFusionCharts, push, render, renderAt, resizeTo, sender,
    setAttribute, setChartAttribute, setChartData, text, trigger,
    updateFusionCharts, width, jQuery
*/
 ...
 * FusionCharts jquery plugin. This module contains the methods to extend the
 * jQuery framework to support methods for performing various operations on
 * FusionCharts object.
 * -----------------------------------------------------------------------------
 **/
...
(function () {
    ...
    var global = FusionCharts(['private', 'jQueryPlugin']);
     ...
     */
    var renderFusionCharts = function (elemList, chartOptions) {
        var i, l, chartObj;
        // Iterating over the element list in reverse order for better
        // performance.
        for (i = 0, l = elemList.length; i < l; i += 1) {
            ...
            if (elemList[i].parentNode) {
                ...
                global.core.render(jQ.extend({}, chartOptions, {
                    renderAt: elemList[i]
                }));
            } else {
                chartObj = new FusionCharts(jQ.extend({}, chartOptions, {
                    renderAt: elemList[i]
                }));
     ...
     */
    var captureAllFusionChartsEvents = function (event, args) {
    ...
    jQ.fn.updateFusionCharts = function (options) {
        ...
        var i, l, fcChart, renderFlag, optStr, filterOpts = {},
            chartObjects = getChartObjectsFromSelection(this);
